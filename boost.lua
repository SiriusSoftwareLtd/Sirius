...
if not request then
	...
	if not requestSuccess or type(requestResult) ~= "table" then
		return
	end
...
local decodeSuccess, boosts = pcall(function()
	return httpService:JSONDecode(response)
end)

if not decodeSuccess or type(boosts) ~= "table" then
	return
end
...
local function display(userId, booster)
	...
	if not target or not booster then
		return
	end
